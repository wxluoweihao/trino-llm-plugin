...
//    }
    ...
    public void huggingFace() throws IOException, InterruptedException {
        ...
        Request request = new Request.Builder()
                .url(apiUrl)
                .post(RequestBody.create(MediaType.parse("application/json"), "{\"inputs\":\"What is the file type of this path： E:/projects/trino-llvm/src/test/resources/example-data/users.csv. Please answer in single word. \"}"))
        ...
        System.out.println("########################## " + responseObject.get(0).getGeneratedText().split("\\n")[1]);
    }

    @Test
    public void sentimenAnalysis() throws IOException, InterruptedException {
        String apiUrl = "https://api-inference.huggingface.co/models/cardiffnlp/twitter-roberta-base-sentiment-latest";
        ...
        Request request = new Request.Builder()
                .url(apiUrl)
                .post(RequestBody.create(MediaType.parse("application/json"), String.format("{\"inputs\":\"you are so bad\"}")))
                .addHeader("Authorization", "Bearer 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")
                .addHeader("Content-Type", "application/json")
                .build();
